...
class ObservationSixScreen extends Component {
  state = {
    photos: []
  };
  ...
  handleNextButton = () => {
    ...
    if (photos.length === 0) {
      navigate("ObservationSeven");
    } else {
      const [, , , , , question] = questions;
      const filterObservationData = temporaryData.observation.filter(
        observation => observation.screen_number !== "6"
      );
      ...
      console.log("FILE NAME", image_file);
      const sendData = {
        ...temporaryData,
        upload_files: [
          ...filterUploadFiles,
